...
public:
    ...
    virtual bool ConnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &coins, bool fJustCheck=false);
    //virtual bool DisconnectBlock(CBlock& block, CTxDB& txdb, CBlockIndex* pindex);
    virtual bool DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &coins, bool *pfClean = NULL);
    ...
    virtual void AcceptToMemoryPool(CValidationState &state, bool fCheckInputs, bool fLimitFree, bool* pfMissingInputs);
...
int GetTxPosHeight(const CDiskTxPos& txPos)
{
    // Read block header
    //~ CBlock block;
    //~ if (!block.ReadFromDisk(txPos.nFile, txPos.nPos, false))
        //~ return 0;
        ...
        //~ CRITICAL_BLOCK(pwalletMain->cs_mapWallet)
        {
            ...
            loop
            {
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int)& coin, vecCoins)
                {
                    ...
                    dPriority += (double)nCredit * (coin.first->GetDepthInMainChain()+1);
                }
                ...
                dPriority += (double)nWtxinCredit * (wtxIn.GetDepthInMainChain()+1);
...
string SendMoneyWithInputTx(CScript scriptPubKey, int64 nValue, int64 nNetFee, CWalletTx& wtxIn, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    string strError;
    if (!CreateTransactionWithInputTx(vecSend, wtxIn, nTxOut, wtxNew, reservekey, nFeeRequired, strError))
    {
        //~ string strError;
...
void CNamecoinHooks::AcceptToMemoryPool(CValidationState &state, bool fCheckInputs, bool fLimitFree, bool* pfMissingInputs)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return;
...
bool CNamecoinHooks::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    return true;
}

//bool CNamecoinHooks::DisconnectBlock(CBlock& block, CTxDB& txdb, CBlockIndex* pindex)
bool CNamecoinHooks::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &coins, bool *pfClean = NULL)
{
    return true;
}
